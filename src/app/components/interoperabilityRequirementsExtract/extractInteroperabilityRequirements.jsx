...
export default function ExtractInteroperabilityRequirements({
    ...
    //Metodo auxiliar para descobrir o nome do contrato caso exista
    // Input pois o contrato sempre é lido
    ...
    for (let messageFlow of messageFlows) {
        ...
        switch (originSplit[0]) {
            case 'Activity':
                ...
                var activitySendMultiplicity = originItem.getElementsByTagName("bpmn:multiInstanceLoopCharacteristics");
                if (activitySendMultiplicity.length === 1) {
                    quantityMessagesSent = "várias mensagens";
                } else {
                    quantityMessagesSent = "uma mensagem";
                }
                // evento de borda tempo

                // evento de borda erro
            ...
            case 'Participant':
                ...
                var participantMultiplicity = originItem.getElementsByTagName("bpmn:participantMultiplicity");
                if (participantMultiplicity.length === 1) {
                    quantityMessagesSent = "várias mensagens";
                } else {
                    quantityMessagesSent = "uma mensagem";
                }
        ...
        switch (destinySplit[0]) {
            case 'Activity':
                ...
                //Verificar multiplicidade
                var activityReceiveMultiplicity = destinyItem.getElementsByTagName("bpmn:multiInstanceLoopCharacteristics");
                if (activityReceiveMultiplicity.length === 1) {
                    quantityMessagesReceive = "várias mensagens";
                } else {
            ...
            case 'Participant':
               ...
               //Verificar multiplicidade no participante está em uma tag interna então basta fazer o get
                var participantReceiveMultiplicity = destinyItem.getElementsByTagName("bpmn:participantMultiplicity");
                if (participantReceiveMultiplicity.length === 1) {
                    quantityMessagesReceive = "várias mensagens";
                } else {
                    quantityMessagesReceive = "uma mensagem";
                }
